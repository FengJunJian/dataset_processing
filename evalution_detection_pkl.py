...
def main():
    ...
    output_dir = get_output_dir(imdb.name, log_path, model_name)  # 创建文件
    ...
    '''
    file_root='../dataset_processing/'
    pkl_file=file_root+model_name+'_detections{}{}.pkl'.format(test_num,'')
    pkl_file=file_root+'yolo_detections_an1300_2.pkl'
    with open(pkl_file, 'rb') as f:
        all_boxes = pickle.load(f)
    print('Evaluating detections')
    aps = imdb.evaluate_detections(all_boxes, file_root+output_dir)
    aps1 = [v for i, v in enumerate(aps) if i != 6]
    with open(os.path.join(file_root+output_dir, 'ap_' + '.txt'), 'w') as f:
        f.write(str(aps) + '\n')
